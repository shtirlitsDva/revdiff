...
// TestReadFileAsContext_NonRegularFile_Directory is the cross-platform half
// of the original non-regular-file test. FIFO and symlink subtests live in
// fallback_unix_test.go (build-tagged !windows) — syscall.Mkfifo doesn't exist
// on Windows, and creating symlinks there requires a privilege most CI hosts
// don't grant.
func TestReadFileAsContext_NonRegularFile_Directory(t *testing.T) {
	dir := t.TempDir()
