...
func TestModel_handleFileLoaded_autoEnablesTableModeForMarkdown(t *testing.T) {
	...
	result, _ := m.Update(fileLoadedMsg{file: "doc.md", lines: lines, seq: m.file.loadSeq})
	model := result.(Model)

	assert.True(t, model.modes.tableMode, "tableMode must auto-enable for .md files")
	require.Len(t, model.file.tableFormatted, len(lines))
	// the three table-row indices should have non-empty formatted strings
	assert.NotEmpty(t, model.file.tableFormatted[2])
	assert.NotEmpty(t, model.file.tableFormatted[3])
	assert.NotEmpty(t, model.file.tableFormatted[4])
	// non-table indices are empty
	assert.Empty(t, model.file.tableFormatted[0])
	assert.Empty(t, model.file.tableFormatted[1])
}
...
func TestModel_handleFileLoaded_disablesTableModeForNonMarkdown(t *testing.T) {
	...
	result, _ := m.Update(fileLoadedMsg{file: "main.go", lines: lines, seq: m.file.loadSeq})
	model := result.(Model)

	assert.False(t, model.modes.tableMode, "tableMode must default off for non-md files")
	assert.Nil(t, model.file.tableFormatted)
}
...
func TestModel_toggleTableMode_roundTrips(t *testing.T) {
	...
	result, _ := m.Update(fileLoadedMsg{file: "doc.md", lines: lines, seq: m.file.loadSeq})
	model := result.(Model)
	require.True(t, model.modes.tableMode)
	require.NotNil(t, model.file.tableFormatted)
	...
	assert.False(t, model.modes.tableMode)
	assert.Nil(t, model.file.tableFormatted, "tableFormatted should clear when toggled off")

	// toggle back on
	model.toggleTableMode()
	assert.True(t, model.modes.tableMode)
	require.NotNil(t, model.file.tableFormatted)
	assert.NotEmpty(t, model.file.tableFormatted[0])
}
...
func TestModel_prepareLineContent_tableFormattedWinsOverHighlight(t *testing.T) {
	...
	model.file.highlighted = []string{"FAKE-HL-0", "FAKE-HL-1", "FAKE-HL-2"}

	_, textContent, hasHL := model.prepareLineContent(0, model.file.lines[0])
	assert.True(t, hasHL)
...
func TestModel_wrapModeDisablesTableFormatting(t *testing.T) {
	...
	assert.True(t, model.modes.tableMode)
	assert.Nil(t, model.file.tableFormatted, "wrapMode must suppress table formatting")
}
...
func TestModel_renderDiff_outputsTableSeparators(t *testing.T) {
	...
	m.layout.viewport = viewport.New(80, 20)
