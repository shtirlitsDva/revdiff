...
Location: `%APPDATA%\revdiff\config` (INI format). Override with `--config` flag or `$env:REVDIFF_CONFIG` env var.
...
```powershell
New-Item -ItemType Directory -Force "$env:APPDATA\revdiff" | Out-Null
revdiff --dump-config > "$env:APPDATA\revdiff\config"
```
...
| `--theme` | `REVDIFF_THEME` | Load color theme from `%APPDATA%\revdiff\themes\` | |
...
| `--history-dir` | `REVDIFF_HISTORY_DIR` | Directory for review history auto-saves | `%APPDATA%\revdiff\history\` |
| `--keys` | `REVDIFF_KEYS` | Path to keybindings file | `%APPDATA%\revdiff\keybindings` |
| `--dump-keys` | | Print effective keybindings to stdout and exit | |
| `--config` | `REVDIFF_CONFIG` | Path to config file | `%APPDATA%\revdiff\config` |
| `--dump-config` | | Print default config to stdout and exit | |

## Split Pane Size (Claude Code plugin)

When launched via the Claude Code plugin skill, revdiff opens in a WezTerm split pane. The split size is configurable via env vars:

| Env var | Description | Default |
|---------|-------------|---------|
| `REVDIFF_POPUP_HEIGHT` | WezTerm split percent (height of new pane vs parent) | `90` |
| `REVDIFF_POPUP_WIDTH` | Accepted for signature parity; the WezTerm launcher uses only height | (ignored) |

## Themes

Eight bundled themes: **basic**, **catppuccin-latte**, **catppuccin-mocha**, **dracula**, **gruvbox**, **nord**, **revdiff**, **solarized-dark**. Stored in `%APPDATA%\revdiff\themes\`, auto-created on first run.

Press `ctrl+t` inside revdiff to open the interactive theme selector with live preview — browse themes, see colors applied instantly, and persist your choice on confirm. (Upstream uses `T` for the theme selector; this fork rebinds `T` to the markdown-table toggle and moves theme-select to `ctrl+t`.)
...
revdiff --dump-theme > "$env:APPDATA\revdiff\themes\my-custom"  # export current colors
```

Set default theme in config: `theme = dracula`. Or env: `$env:REVDIFF_THEME = 'dracula'`.

**Custom themes:** customize colors in config or via `--color-*` flags, then `revdiff --dump-theme > "$env:APPDATA\revdiff\themes\my-custom"`. Or copy a bundled theme file and edit directly — each has all 24 color keys + `chroma-style`.

Precedence: `--theme` takes over completely — overwrites all color fields, ignoring `--color-*` flags and env vars. Without `--theme`: built-in defaults → config file → env vars → CLI flags. `--theme` + `--no-colors` prints warning and applies theme. `color-table-code` is optional in theme files: themes without it lose the inline-code coloring inside markdown tables (cells render as plain text); the rest of the table reformatting still works.
...
| `--color-table-code` | Inline `` `code` `` foreground inside reformatted markdown tables | `#87afd7` |

## Chroma Syntax Highlighting Styles

Set via `--chroma-style=<name>`, env var `$env:REVDIFF_CHROMA_STYLE`, or config file `chroma-style = <name>`.
...
Location: `%APPDATA%\revdiff\keybindings`. Override with `--keys` flag or `$env:REVDIFF_KEYS`.
...
Generate a template: `revdiff --dump-keys > "$env:APPDATA\revdiff\keybindings"`
