...
revdiff --only=$env:TEMP\plan.md     # review a file outside a repo (context-only)
revdiff --only=docs\notes.txt        # review a file with no VCS changes (context-only)
revdiff --compare-old=$env:TEMP\plan-old.md --compare-new=docs\plans\plan.md  # diff two arbitrary files (no VCS needed)
'# Plan`n`nBody`n' | revdiff --stdin --stdin-name plan.md         # review piped text as markdown
some-command | revdiff --stdin --output $env:TEMP\annotations.txt  # annotate generated output
Get-Content notes.md | revdiff --stdin --stdin-name notes.md       # pipe a file through stdin
```
...
**XML-style structural headings** (this fork): in addition to `#`-style ATX headers, the TOC also recognizes XML-style structural tags like `<overview>` or `<example type="good">` when they appear alone on their own trimmed line. The nesting level is computed by a tolerant tag stack, so deeper tags indent further in the TOC. Self-closing tags (`<br/>`), HTML comments, DOCTYPE, and `<?xml?>` processing instructions are ignored.

Recognized file extensions for TOC: `.md`, `.markdown`, `.xml`, `.xhtml`.

## Markdown Table Reformatting

For files with `.md` / `.markdown` extensions, revdiff reformats GitHub-Flavored Markdown tables into column-aligned cells with Unicode box-drawing borders (`├───┼───┤` on the separator row). Inline cell markdown is rendered live: `` `code` `` is colored via `color-table-code`, `**bold**` and `*italic*` use raw ANSI, and `[text](url)` links collapse to their text. GFM alignment markers (`:---`, `---:`, `:---:`) are honored.

Toggle with `T` — auto-on for `.md` / `.markdown` files; the toggle is global. Suppressed automatically when word wrap (`w`) is on, since aligned columns and soft-wrap are incoherent.

**Search caveat**: aligned tables replace ASCII `|` with `│`, so a literal-pipe search misses inside formatted tables. Disable with `T` if you need to search for `|`.
...
`--include` and `--exclude` can be persisted in config file (`include = src`, `exclude = vendor`) or via env vars (`$env:REVDIFF_INCLUDE = 'src'`, `$env:REVDIFF_EXCLUDE = 'vendor,mocks'`). Include narrows first, then exclude removes from the included set.
...
```powershell
revdiff HEAD~3 --description="# Refactor auth middleware`n`nDrop session-token storage to meet new compliance requirements.`nSee ticket SEC-441 for context."

# longer markdown lives in a file
revdiff HEAD~3 --description-file=$env:TEMP\review-notes.md
```
...
```powershell
revdiff --compare-old=$env:TEMP\plan-old.md --compare-new=docs\plans\plan.md
...
- On Windows, revdiff reopens the console for interactive input by opening `CONIN$` (the Windows console device) in read/write mode so Bubble Tea can enter raw mode while stdin is still attached to the pipe

## Key Bindings

This fork rebinds the markdown-table toggle: **`T` is the markdown-table toggle** (preserved from earlier fork behavior), and **`ctrl+t` opens the theme selector** (upstream had this on `T`). Every reference below reflects the fork mapping.
...
| `Ctrl+E` (during annotation input) | Open `$env:EDITOR` for multi-line annotation (`open_editor` — rebindable) |
| `Esc` | Cancel annotation input |

While the annotation input is active, press `Ctrl+E` (or whatever key is bound to `open_editor`) to hand off the current text to an external editor for multi-line comments. Editor resolution: `$env:EDITOR` → `$env:VISUAL` → `vi`. Values with arguments work (e.g. `$env:EDITOR = 'code --wait'`). On editor save and quit, the full file contents (including newlines) become the annotation. Quitting the editor with an empty file cancels the annotation and preserves any previously stored note on that line. Multi-line annotations are rendered line-by-line in the diff view, shown flattened in the annotation list popup (`@`), and emitted with embedded newlines in the structured output.
...
| `T` | Toggle markdown-table reformatting (column-aligned cells, box-drawing borders, inline ANSI for `**bold**` / `*italic*` / `` `code` ``) — auto-on for `.md` / `.markdown` files |
| `ctrl+t` | Open theme selector with live preview (fork rebinding — upstream uses `T` for this) |
...
| `▦` | `T` | Markdown-table reformatting active |
...
**Text selection trade-off** — once mouse tracking is on, plain drag is captured by revdiff. For terminal-native text selection in WezTerm, hold `Shift` while dragging.

Because the tree pane is rendered alongside the diff on the same rows, multi-line Shift+drag will include tree content. For clean copies of diff text, use WezTerm's block-select (Ctrl+Shift+drag) or run with `--no-mouse` to disable mouse capture entirely.

Opt out with `--no-mouse`, `$env:REVDIFF_NO_MOUSE = 'true'`, or `no-mouse = true` in the config file.

## Custom Keybindings

All keybindings can be customized via `%APPDATA%\revdiff\keybindings` (override path with `--keys` or `$env:REVDIFF_KEYS`).
...
Generate a template with all defaults: `revdiff --dump-keys > $env:APPDATA\revdiff\keybindings`
...
Opt-in vim-style motion layer activated via `--vim-motion`, `$env:REVDIFF_VIM_MOTION = 'true'`, or `vim-motion = true` in the config file. Off by default — when off, existing single-key bindings are unchanged.
...
When you quit with annotations (`q`), revdiff automatically saves a copy of the review session to `%APPDATA%\revdiff\history\<repo-name>\<timestamp>.md`. This is a safety net — if annotations are lost (process crash, agent fails to capture stdout), the history file preserves them.
...
Override the history directory with `--history-dir`, `$env:REVDIFF_HISTORY_DIR`, or `history-dir` in the config file.
